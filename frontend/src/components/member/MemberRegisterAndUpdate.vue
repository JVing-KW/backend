        ...
        <div class="col-md-6 col-lg-5">
          ...
          <form @submit.prevent="submitForm" id="frmSignUp">
            ...
            <div class="row">
              <div class="form-group col">
                ...
                <input
                  type="password"
                  v-model="mbrPswrd"
                  @blur="touched.mbrPswrd = true"
                  class="form-control form-control-lg text-4"
                  required
                  placeholder="영문, 숫자를 포함한 4자 이상 20자 이내"
                />
                <div v-if="touched.mbrPswrd && errorPswrd" class="error">
                  {{ errorPswrd }}
                </div>
              </div>
            </div>
            <div class="row">
              <div class="form-group col">
                <label class="form-label text-color-dark text-3"
                  >비밀번호 확인 <span class="text-color-danger">*</span></label
                >
                <input
                  type="password"
                  v-model="pswrdConfirm"
                  @blur="touched.pswrdConfirm = true"
                  class="form-control form-control-lg text-4"
                  required
                />
                <div
                  v-if="touched.pswrdConfirm && errorPswrdConfirm"
                  class="error"
                >
                  {{ errorPswrdConfirm }}
                </div>
            ...
            <div class="row">
              <div class="form-group col">
                ...
                <input
                  type="text"
                  v-model="mbrName"
                  @blur="touched.mbrName = true"
                  class="form-control form-control-lg text-4"
                  required
                />
                <div v-if="touched.mbrName && errorMbrName" class="error">
                  {{ errorMbrName }}
                </div>
              </div>
            </div>
            <div class="row">
              <div class="form-group col">
                <label class="form-label text-color-dark text-3"
                  >생년월일 <span class="text-color-danger">*</span></label
                >
                <input
                  type="text"
                  v-model="mbrBd"
                  @blur="touched.mbrBd = true"
                  class="form-control form-control-lg text-4"
                  required
                  placeholder="8자리"
                />
                <div v-if="touched.mbrBd && errorMbrBd" class="error">
                  {{ errorMbrBd }}
                </div>
              </div>
            </div>
            <div class="row">
              <div class="form-group col">
                ...
                <input
                  type="text"
                  v-model="mbrMp"
                  @blur="touched.mbrMp = true"
                  class="form-control form-control-lg text-4"
                  required
                  placeholder="-(다시)는 제외하고 입력"
                />
                <div v-if="touched.mbrMp && errorMbrMp" class="error">
                  {{ errorMbrMp }}
                </div>
              </div>
            </div>
            <div class="row">
              <div class="form-group col-9">
                ...
                <input
                  type="email"
                  v-model="mbrEmlAdrs"
                  @blur="touched.mbrEmlAdrs = true"
                  class="form-control form-control-lg text-4"
                  required
                />
                <div v-if="touched.mbrEmlAdrs && errorMbrEmlAdrs" class="error">
                  {{ errorMbrEmlAdrs }}
                </div>
              </div>
            ...
            <div class="row">
              ...
              <div v-if="touched.emailKey && errorMbrEmlChck" class="error">
                {{ errorMbrEmlChck }}
              </div>
            </div>
            <div class="row">
            ...
            <div>
              <div
                v-if="errorMbrPrvcyTrmsChck"
                class="error errorMbrPrvcyTrmsChck"
              >
            ...
            <div class="row">
              <div class="form-group col">
                <a
                  ...
                  data-loading-text="Loading..."
                  ><i class="fab fa-facebook text-5 me-2"></i> 소셜 회원가입</a
                >
              </div>
            </div>
          </form>
...
// 비동기 x  그냥 폼으로 보내는게 더 보기 좋을듯
const emlSend = function () {
  emailCode.value = "";

  try {
    const response = sendEmlRegister(mbrEmlAdrs.value);
    if (
...
watchEffect(() => {
  ...
  if (!/^[A-Za-z0-9]{4,20}$/.test(mbrPswrd.value)) {
    errorPswrd.value =
      "비밀번호 형식이 올바르지 않습니다. (영문, 숫자를 포함한 4자 이상 20자 이내)";
  } else {
    errorPswrd.value = "";
  }
...
const submitForm = async () => {
  if (Object.values(touched).some((field) => field === false)) {
